...
const SubmitAiForm = async (req, res) =>{
 try{
  ...
  if (!psychicId) {
    return res.status(400).json ({success:false, message:"Psychics Id not required"})
  }

  const psychic = await aiPsychic.findById(psychicId).select('type');
if(!psychic) {
  return res.status(401).json ({
    success:false, message:"Psychics not found"
  });
  
}
const {type} = psychic
const requriedField = getRequiredFieldsByType(type);
const missingFields = requriedField.filter (f => !formData[f]);

if (missingFields.length >0){
   return res.status(400).json ({ success: false,         message: `Missing required fields: ${missingFields.join(', ')}`
})
}
    // Save or update form data by userId + type
